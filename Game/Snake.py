...
def getdateandtime(name):
    try:
        today = datetime.datetime.now()
        month = today.strftime("%B")
        ndate = today.strftime("%d-%m")
        path = Path("Data/"+str(month)+"/"+str(ndate)+".xlsx")
        print(path)
        if os.path.exists(path):
            df = pd.read_excel(path, index_col=0, engine='openpyxl')
            #print(df)
            Time = df.loc[name]
            return datetime.datetime.strptime(Time[0],"%H:%M:%S")
    except KeyError:
        return None
            
def checkfinalhour(min,name):
    try:
        now = datetime.datetime.strftime(datetime.datetime.now(),"%H:%M:%S")
        # print(type(getdateandtime(name)),getdateandtime(name))
        finaltime = datetime.datetime.strftime(getdateandtime(name)+datetime.timedelta(minutes=min),"%H:%M:%S")
        final = datetime.datetime.strptime(now,"%H:%M:%S")- (datetime.datetime.strptime(finaltime,"%H:%M:%S"))
        final = final.total_seconds()//60
        print(final)
        if int(final)< 0:
            return True
        else:
...
#GameOverScreen
           ...
           if snake_x<0 or snake_x>screen_width or snake_y<0 or snake_y>screen_height:
               ...
               pygame.mixer.music.play(100)
               pygame.mixer.music.set_volume(.6)
           print(checkfinalhour(int(gettime()),getname()))
           if checkfinalhour(int(gettime()),getname()) == False:
            game_over = True
